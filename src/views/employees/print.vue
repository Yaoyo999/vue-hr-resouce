  ...
  <div class="dashboard-container" id="myPrint">
    <div class="app-container">
      <el-card>
        ...
        <div v-if="type === 'personal'" id="printId">
...
<script>
// 打印功能的流程1.下载安装包 npm i vue-print-nb 2.全局注册 import Print from 'vue-print-nb' Vue.use(Print) 使用v-print指令绑定要打印的盒子 盒子需要绑定id属性 v-print="printId" data () { return { printId: { id: 盒子的id }} 前者写法只会打印盒子内的信息或者 printId: 盒子的id 这种写法会打印整个浏览页面} 

import { getPersonDetail, getPersonJob } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getPersonDetail() : this.getPersonJob()
  },
  // 组件更新
  methods: {
    async getPersonDetail() {
      const userBaseInfo = await getBaseUserInfo(this.userId) // 获取员工基本信息
      const userInfo = await getPersonDetail(this.userId) // 获取员工个人信息
      this.formData = {...userBaseInfo, ...userInfo}

    },
    async getPersonJob() {
      const userBaseInfo = await getBaseUserInfo(this.userId) // 获取员工基本信息
      const userJobInfo = await getPersonJob(this.userId) // 获取岗位基本信息
      this.formData = {...userBaseInfo, ...userJobInfo}
    }
  }
}
</script>
