      ...
      <div class="block">
        <div class="title">基础信息</div>
         <el-row type="flex" justify="end">
          <el-tooltip effect="dark" content="打印岗位信息" placement="top-start">
            <router-link :to="`/employees/print/${this.userId}/?type=job`">
...
import { getPersonJob, savePersonJob, getEasyEmployee } from '@/api/employees'
export default {
  name: 'job-info',
  data() {
    return {
  ...
  created(){
    this.getPersonJob()
    this.getEasyEmployee()
  },
  methods: {
   async getPersonJob() {
  this.formData = await getPersonJob(this.userId)
    },
    // 保存岗位信息
   async save () {
     await savePersonJob(this.formData)
     this.$message.success('保存成功')
    },
    async getEasyEmployee() {
    this.depts = await getEasyEmployee()
    }
  }
}
</script>
